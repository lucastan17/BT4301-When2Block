<template>
  ...
  <input
    ...
    v-model="timing.time"
    list="timing"
    placeholder="Up till next 2 hours"
  />
  <datalist id="timing">
    <option
  ...
  <o-button
    size="small"
    class="button"
    style="margin: 15px 15px 15px 0px; font-weight: bold;"
    @click="reset()"
  > Clear
    Search</o-button>
  ...
  >
    ...
    <div style="display:flex; justify-content: center;">
      ...
      <img
        v-show="!isSunny"
        alt="Sunscreen gray Logo"
        src="../assets/sunscreen-gray.png"
        width="150"
        style="align-items:center; padding: 10px 10px 20px 0px; margin:15px"
      />
      <h2
      ...
      > It's going to be SUNNY <br> Wear your
        Sunscreen! </h2>
  ...
  <div id="maps">
    ...
    >
      ...
      <l-marker
        v-for="(item,index) in this.infometa"
        :key="index"
...
export default {
  ...
  data() {
    return {
      dummyModel: {
        model_id: 1,
        location: "testmodel",
        time: new Date(new Date().getTime() + 8 * (3600 * 1000)),
        weather: "Sunny",
        uv_index: 3,
        prediction: 1,
        actual: 1,
      ...
      modJ: null,
      places: {
        "Ang Mo Kio": { name: "Ang Mo Kio", forecast: "1", lat: "", long: "" },
        Bedok: { name: "Bedok", forecast: "", lat: "", long: "" },
        Bishan: { name: "Bishan", forecast: "", lat: "", long: "" },
        "Boon Lay": { name: "Boon Lay", forecast: "", lat: "", long: "" },
        "Bukit Batok": { name: "Bukit Batok", forecast: "", lat: "", long: "" },
        "Bukit Merah": {
          name: "Bukit Merah",
          forecast: "2",
          lat: "",
        ...
        "Bukit Timah": { name: "Bukit Timah", forecast: "", lat: "", long: "" },
        ...
        Changi: { name: "Changi", forecast: "", lat: "", long: "" },
        "Choa Chu Kang": {
          name: "Choa Chu Kang",
          forecast: "3",
          lat: "",
        ...
        Clementi: { name: "Clementi", forecast: "", lat: "", long: "" },
        City: { name: "City", forecast: "", lat: "", long: "" },
        Geylang: { name: "Geylang", forecast: "", lat: "", long: "" },
        Hougang: { name: "Hougang", forecast: "", lat: "", long: "" },
        "Jalan Bahar": {
          name: "Jalan Bahar",
          forecast: "4",
          lat: "",
        ...
        "Jurong East": { name: "Jurong East", forecast: "", lat: "", long: "" },
        "Jurong Island": {
          name: "Jurong Island",
          forecast: "",
          lat: "",
        ...
        "Jurong West": { name: "Jurong West", forecast: "", lat: "", long: "" },
        Kallang: { name: "Kallang", forecast: "", lat: "", long: "" },
        "Lim Chu Kang": {
          name: "Lim Chu Kang",
          forecast: "",
          lat: "",
        ...
        Mandai: { name: "Mandai", forecast: "", lat: "", long: "" },
        "Marine Parade": {
          name: "Marine Parade",
          forecast: "",
          lat: "",
        ...
        Novena: { name: "Novena", forecast: "", lat: "", long: "" },
        "Pasir Ris": { name: "Pasir Ris", forecast: "", lat: "", long: "" },
        "Paya Lebar": { name: "Paya Lebar", forecast: "", lat: "", long: "" },
        Pioneer: { name: "Pioneer", forecast: "", lat: "", long: "" },
        "Pulau Tekong": {
          name: "Pulau Tekong",
          forecast: "",
          lat: "",
        ...
        "Pulau Ubin": { name: "Pulau Ubin", forecast: "", lat: "", long: "" },
        Punggol: { name: "Punggol", forecast: "", lat: "", long: "" },
        Queenstown: { name: "Queenstown", forecast: "", lat: "", long: "" },
        Seletar: { name: "Seletar", forecast: "", lat: "", long: "" },
        Sembawang: { name: "Sembawang", forecast: "", lat: "", long: "" },
        Sengkang: { name: "Sengkang", forecast: "", lat: "", long: "" },
        Sentosa: { name: "Sentosa", forecast: "", lat: "", long: "" },
        Serangoon: { name: "Serangoon", forecast: "", lat: "", long: "" },
        "Southern Islands": {
        ...
        Tampines: { name: "Tampines", forecast: "", lat: "", long: "" },
        Tanglin: { name: "Tanglin", forecast: "", lat: "", long: "" },
        Tengah: { name: "Tengah", forecast: "", lat: "", long: "" },
        "Toa Payoh": { name: "Toa Payoh", forecast: "", lat: "", long: "" },
        Tuas: { name: "Tuas", forecast: "", lat: "", long: "" },
        "Western Islands": {
        ...
        Woodlands: { name: "Woodlands", forecast: "", lat: "", long: "" },
        Yishun: { name: "Yishun", forecast: "", lat: "", long: "" },
      },
  ...
  methods: {
      ...
      try {
        const response = await axios
          ...
          .catch((err) => {
            console.log(err.message);
            alert(
              "Error getting data from Real-Time API. Please try again later"
            );
          });
        const response2 = await axios
          .get("https://api.data.gov.sg/v1/environment/uv-index")
          .catch((err) => {
            console.log(err.message);
            alert(
              "Error getting data from Real-Time API. Please try again later"
    ...
    fillData() {
      ...
      for (var j = 0; j < 3; j++) {
        var cT = new Date();
        var display = new Date(cT.getTime() + (j + 8) * 3600 * 1000)
          .toISOString()
          .substring(11, 16);
    ...
    async runUVI() {
      ...
      const weightData = new Uint8Array(Buffer.from(json.weightData, "base64"))
        .buffer;
      const UVImodel = await tf.loadLayersModel(
    ...
    async runPred(uviPred, weatherPred) {
      ...
      const weightData = new Uint8Array(Buffer.from(json.weightData, "base64"))
        .buffer;
      const predModel = await tf.loadLayersModel(
        tf.io.fromMemory(json.modelTopology, json.weightSpecs, weightData)
      );
    ...
    async weatherPredData(hour) {
      ...
      console.log(hour);
      const weatherDataObj = await axios.get(
    ...
    async searchBody() {
      ...
      const hour = parseInt(
        document.getElementById("select-time").value.substring(0, 2)
      );
      ...
      if (off_hours.includes(hour)) {
        ...
        for (var i = 0; i < 47; i++) {
          ...
          const ts = new Date(
            new Date().getTime() + (8 + diff) * (3600 * 1000)
          );
          ...
          const response = await searchService.post({
            ...
            actual:
              this.sunnyConditions.includes(weatherPred[i].forecast) &
              (uviPred > 3)
                ? 1
                : 0,
        ...
        const locFilter = dbPred.filter(function (data) {
          return data.location == loc;
        });
        const timeFilter = locFilter.filter(function (data) {
          return data.time.substring(11, 13) == hour;
        });
